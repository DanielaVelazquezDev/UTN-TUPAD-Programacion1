Este repositorio ha sido creado como parte del Trabajo Práctico: uso de Git mediante GitHub y GitHub Desktop para la materia Programación I de la Tecnicatura Universitaria en Programación.

📌 Objetivo del Proyecto
El propósito de este trabajo es aplicar los conocimientos adquiridos sobre el control de versiones, utilizando herramientas como GitHub y GitHub Desktop.

📂 Contenido
En este repositorio se encuentran cargados los archivos correspondientes a la práctica de la unidad anterior. Estos códigos fueron desarrollados previamente y se integran aquí para demostrar el flujo de trabajo básico en Git:
  Clonación del repositorio.Gestión de archivos locales.Realización de commits con mensajes descriptivos.Push de los cambios al repositorio remoto.
  
🛠️ Tecnologías utilizadas
Git (Control de versiones)
GitHub Desktop (Cliente de escritorio)
GitHub (Alojamiento de repositorio remoto)
